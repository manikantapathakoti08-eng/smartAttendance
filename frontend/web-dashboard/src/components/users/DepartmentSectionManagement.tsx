    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="w-full max-w-md glass-modal p-6">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Department Name</label>
            <Input 
              value={name}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Department Code</label>
            <Input 
              value={code}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-900 focus:outline-none focus:border-primary/50 transition-colors min-h-[100px] placeholder:text-slate-400"
          ...
          <div className="flex items-center space-x-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
            ...
            />
            <label htmlFor="dept-active" className="text-sm text-slate-600 cursor-pointer">Mark as Active</label>
          </div>
      ...
      <div className="w-full max-w-md glass-modal p-6">
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto px-1 pr-2">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1.5">Section Name</label>
              <Input 
                value={name}
                onChange={(e) => setName(e.target.value)}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1.5">Batch Year</label>
              <Input 
                type="number"
                value={batchYear}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Program / Degree</label>
            <Input 
              value={program}
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1.5">Total Years</label>
              <Input 
            ...
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1.5">Semester</label>
              <Input 
                type="number"
          ...
          <div>
            ...
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-900 focus:outline-none focus:border-primary/50 transition-colors min-h-[80px] placeholder:text-slate-400"
          ...
          <div className="flex items-center space-x-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
            <input 
              ...
              className="w-4 h-4 rounded border-slate-300 bg-white text-primary focus:ring-primary"
            />
            <label htmlFor="section-active" className="text-sm text-slate-600 cursor-pointer">Mark as Active</label>
          </div>
      ...
      <div className="w-full max-w-2xl glass-modal overflow-hidden flex flex-col max-h-[80vh]">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-primary/10 to-transparent">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">{deptName}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center space-y-4">
              <Loading size="lg" />
              <p className="text-slate-500 animate-pulse">Fetching department experts...</p>
            </div>
          ) : faculty.length === 0 ? (
            <div className="py-20 text-center">
              <Users className="mx-auto text-slate-200 mb-4" size={64} />
              <p className="text-slate-500 font-medium text-lg">No faculty mapped to this department yet.</p>
              <p className="text-sm text-slate-400 mt-2 italic">Assign faculty members to see them here.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {faculty.map((member, idx) => (
                <div 
                  key={member.id || idx}
                  className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between hover:border-primary/30 transition-all group"
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 className="text-slate-900 font-semibold">{member.name}</h4>
                      <p className="text-xs text-slate-500">{member.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="text-[10px] uppercase font-bold tracking-tighter px-2 py-0.5 bg-primary/10 text-primary border border-primary/20 rounded-full">
      ...
      <div className="w-full max-w-2xl glass-modal overflow-hidden flex flex-col max-h-[80vh]">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-emerald-500/10 to-transparent">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">{sectionName}</p>
          </div>
          <button 
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center space-y-4">
              <Loading size="lg" />
              <p className="text-slate-500 animate-pulse">Fetching class list...</p>
            </div>
          ) : students.length === 0 ? (
            <div className="py-20 text-center">
              <Users className="mx-auto text-slate-200 mb-4" size={64} />
              <p className="text-slate-500 font-medium text-lg">No students assigned to this section yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {students.map((student, idx) => (
                <div 
                  key={student.id || idx}
                  className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between hover:border-emerald-500/30 transition-all group"
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h4 className="text-slate-900 font-semibold">{student.name}</h4>
                      <p className="text-xs text-slate-500">{student.email}</p>
                      <p className="text-[10px] text-slate-400 font-mono mt-1">{student.registrationNumber}</p>
                    </div>
                  </div>
                </div>
              ))}
    ...
    <div className="space-y-6 pb-20">
      {viewMode === 'grid' ? (
        <div className="space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-1">Manage institutional departments, faculty allocations, and student statistics</p>
            </div>
          ...
          {loading ? (
            ...
            <Card glass className="py-20 flex flex-col items-center justify-center text-center border-dashed">
              <Building className="text-slate-200 mb-4" size={64} />
              <h3 className="text-xl font-bold text-slate-900">No Departments Found</h3>
              <p className="text-slate-500 mt-2 max-w-sm">Start by creating your first academic department to manage sections and students.</p>
              ...
              {filteredDepts.map((dept) => (
                ...
                >
                  ...
                  <div className="space-y-1 mb-8">
                    <h3 className="text-xl font-bold text-slate-900 group-hover:text-primary transition-colors line-clamp-1">{dept.name}</h3>
                    <div className="flex items-center space-x-2">
                      <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500 bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
                        {dept.code}
                      </span>
                      {!dept.isActive && (
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center justify-center text-center">
                      <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Students</span>
                      <span className="text-2xl font-black text-slate-900">{dept.studentCount || 0}</span>
                    </div>
                    <div 
                      onClick={() => handleViewFaculty(dept)}
                      className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-primary/10 hover:border-primary/30 transition-all group/stat"
                    >
                      <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1 group-hover/stat:text-primary">Faculty</span>
                      <span className="text-2xl font-black text-primary group-hover/stat:scale-110 transition-transform">{dept.facultyCount || 0}</span>
                    </div>
                  </div>

                  <button 
                    onClick={() => { setSelectedDeptId(dept.id); setViewMode('details'); }}
                    className="w-full py-4 bg-slate-50 hover:bg-primary/10 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-primary border border-slate-100 hover:border-primary/30 rounded-2xl transition-all flex items-center justify-center space-x-2"
                  >
                    <LayoutGrid size={14} />
                  ...
                  {dept.isActive && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-primary rounded-full border-2 border-white shadow-lg shadow-primary/50" />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <div className="text-right">
                <h2 className="text-xl font-bold text-slate-900">{departments.find(d => d.id === selectedDeptId)?.name}</h2>
                <p className="text-[10px] text-primary font-bold tracking-widest uppercase">{departments.find(d => d.id === selectedDeptId)?.code} Operations</p>
              </div>
          ...
          <Card glass className="overflow-hidden flex flex-col border-slate-200">
            <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 py-6">
              <div className="flex items-center space-x-4">
                <div className="p-3 bg-emerald-500/10 text-emerald-400 rounded-2xl">
                  <Layers size={24} />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Academic Sections</h3>
                  <p className="text-xs text-slate-500">Managing capacity and curriculum flow</p>
                </div>
              </div>
                    ...
                    >
                      <div className="absolute top-4 right-4 p-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="p-2 bg-slate-50 text-slate-400 hover:text-slate-900 rounded-xl border border-slate-200"
                        >
                          <Edit size={14} />
                        </button>
                        <button 
                          onClick={() => handleDeleteSection(section.id)}
                          className="p-2 bg-slate-50 text-slate-400 hover:text-red-500 rounded-xl border border-slate-200"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-start mb-6">
                        ...
                        <div className="space-y-0.5">
                          <h4 className="font-bold text-slate-900 text-lg">{section.name}</h4>
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-6">
                        <div className="bg-slate-50 rounded-xl py-2 px-3 border border-slate-100">
                          <span className="text-[8px] text-slate-500 uppercase block mb-1">Batch</span>
                          <span className="text-xs font-bold text-slate-900">{section.batchYear}</span>
                        </div>
                        <div className="bg-slate-50 rounded-xl py-2 px-3 border border-slate-100">
                          <span className="text-[8px] text-slate-500 uppercase block mb-1">Semester</span>
                          <span className="text-xs font-bold text-primary">S{section.currentSemester}</span>
                        </div>
                        <div className="bg-slate-50 rounded-xl py-2 px-3 border border-slate-100">
                          <span className="text-[8px] text-slate-500 uppercase block mb-1">Years</span>
                          <span className="text-xs font-bold text-purple-600">{section.totalAcademicYears}Y</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                        <div 
                          ...
                          className="flex items-center space-x-2 cursor-pointer hover:bg-emerald-50 p-1 rounded-lg transition-colors group/stats"
                        >
                          <Users size={16} className="text-slate-400 group-hover/stats:text-emerald-500" />
                          <span className="text-sm font-bold text-slate-900 group-hover/stats:text-emerald-500">{section.studentCount || 0} / {section.capacity}</span>
